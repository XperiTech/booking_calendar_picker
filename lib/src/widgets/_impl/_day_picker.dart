part of '../booking_calendar_picker.dart';
...
class _DayPickerState extends State<_DayPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.config.calendarViewMode == BookingCalendarPickerMode.scroll &&
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        bool isCheckoutDate = widget.bookingAvailabilities.any(
          (e) =>
              !e.available &&
              e.canCheckout &&
              (e.date.day == dayToBuild.day &&
                  e.date.month == dayToBuild.month &&
                  e.date.year == dayToBuild.year),
        );

        final isDisabled = (dayToBuild.isAfter(widget.config.lastDate) ||
            dayToBuild.isBefore(widget.config.firstDate) ||
            !(widget.config.selectableDayPredicate?.call(dayToBuild) ?? true));

        late final bool shouldIndicateCheckout;

        if (isCheckoutDate && !isDisabled) {
          isCheckoutDate = false;
          shouldIndicateCheckout = true;
        } else {
          shouldIndicateCheckout = false;
        }
        ...
        final isFullySelectedRangePicker =
            widget.config.calendarType == BookingCalendarPickerType.range &&
        ...
        if (isCheckoutDate) {
          customDayTextStyle = widget.config.checkoutOnlyDayTextStyle;
        }
        ...
        if (isDisabled) {
          ...
          var dayInkRadius = _dayPickerRowHeight / 2;
          ...
          dayWidget = InkResponse(
            ...
            child: Semantics(
              ...
              selected: shouldIndicateCheckout ? true : false,
      ...
      children: [
        ...
        AspectRatio(
          ...
          child: InkWell(
            ...
            onTap: isCheckoutDate ? widget.onCheckoutOnlyClick : null,
            child: Container(
...
class _DayPickerGridDelegate extends SliverGridDelegate {
  ...
  SliverGridLayout getLayout(SliverConstraints constraints) {
    ...
    if (config?.calendarViewMode == BookingCalendarPickerMode.scroll &&
