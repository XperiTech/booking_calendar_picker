...
class BookingCalendarPicker extends StatefulWidget {
  BookingCalendarPicker({
    ...
    required this.value,
    required this.bookingAvailabilities,
    this.onValueChanged,
    this.displayedMonthDate,
    this.onDisplayedMonthChanged,
    required this.onCheckoutOnlyClick,
    Key? key,
  }) : super(key: key) {
    const valid = true;
    const invalid = false;

    if (config.calendarType == BookingCalendarPickerType.single) {
      assert(value.length < 2,
          'Error: single date picker only allows maximum one initial date');
    }

    if (config.calendarType == BookingCalendarPickerType.range &&
        value.length > 1) {
  ...
  final BookingCalendarPickerConfig config;
  ...
  /// Called when the user picks a chackout day.
  final VoidCallback onCheckoutOnlyClick;

  /// The availability calend list
  final List<BookingAvailability> bookingAvailabilities;
...
class _BookingCalendarPickerState extends State<BookingCalendarPicker> {
  ...
  late BookingCalendarPickerMode _mode;
  ...
  @override
  void didUpdateWidget(BookingCalendarPicker oldWidget) {
  ...
  void _handleModeChanged(BookingCalendarPickerMode mode) {
    _vibrate();
    setState(() {
      _mode = mode;
      if (_selectedDates.isNotEmpty) {
        for (final date in _selectedDates) {
          if (date != null) {
            SemanticsService.announce(
              _mode == BookingCalendarPickerMode.day
                  ? _localizations.formatMonthYear(date)
                  : _mode == BookingCalendarPickerMode.month
                      ? _localizations.formatMonthYear(date)
  ...
  void _handleDayChanged(DateTime value) {
    ...
    setState(() {
      ...
      switch (calendarType) {
        case BookingCalendarPickerType.single:
          selectedDates = [value];
          break;

        case BookingCalendarPickerType.multi:
          final index =
        ...
        case BookingCalendarPickerType.range:
      ...
      final isValueDifferent =
          widget.config.calendarType != BookingCalendarPickerType.single ||
  ...
  Widget _buildPicker() {
    switch (_mode) {
      case BookingCalendarPickerMode.day:
        return _CalendarView(
          ...
          bookingAvailabilities: widget.bookingAvailabilities,
          onCheckoutOnlyClick: widget.onCheckoutOnlyClick,
        );
      case BookingCalendarPickerMode.month:
        return Padding(
      ...
      case BookingCalendarPickerMode.year:
        return Padding(
          padding: EdgeInsets.only(
              top: widget.config.controlsHeight ?? _subHeaderHeight),
          child: YearPicker(
      ...
      case BookingCalendarPickerMode.scroll:
        return Container(
  ...
  Widget build(BuildContext context) {
    ...
    return widget.config.calendarViewMode == BookingCalendarPickerMode.scroll
        ? _buildPicker()
        : Stack(
            children: <Widget>[
              SizedBox(
              ...
              _DatePickerModeToggleButton(
                ...
                onMonthPressed: () {
                  if (_mode == BookingCalendarPickerMode.year) {
                    _handleModeChanged(BookingCalendarPickerMode.month);
                  } else {
                    _handleModeChanged(
                      _mode == BookingCalendarPickerMode.month
                          ? BookingCalendarPickerMode.day
                          : BookingCalendarPickerMode.month,
                    );
                  }
                },
                onYearPressed: () {
                  if (_mode == BookingCalendarPickerMode.month) {
                    _handleModeChanged(BookingCalendarPickerMode.year);
                  } else {
                    _handleModeChanged(
                      _mode == BookingCalendarPickerMode.year
                          ? BookingCalendarPickerMode.day
                          : BookingCalendarPickerMode.year,
                    );
                  }
                },
              ),
